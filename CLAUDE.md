...
- **ESP32 Firmware** (`services/esp32/`): Single codebase targeting both ESP32-S3 and ESP32-WROOM.
  ...
  - Wired Switch Pro mode (S3) now uses a per-device USB serial number derived from efuse MAC and returns that same MAC in Switch USB handshake / device-info replies for stable host identity while keeping legacy Pro Controller USB VID/PID for compatibility.
