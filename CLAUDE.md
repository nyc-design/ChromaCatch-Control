...
- **ESP32 Firmware** (`services/esp32/`): Single codebase targeting both ESP32-S3 and ESP32-WROOM.
  ...
  - Wired Switch Pro mode now enforces USB build compatibility at runtime: firmware built with `ARDUINO_USB_CDC_ON_BOOT=1` rejects `wired_switch_pro_controller` with a clear error/hint to use `esp32s3_switch` (CDC off), and mode transitions that change USB descriptor families now trigger an automatic reboot to apply descriptors cleanly.
