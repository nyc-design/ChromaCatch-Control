...
- **ESP32 Firmware** (`services/esp32/`): Single codebase targeting both ESP32-S3 and ESP32-WROOM.
  ...
  - Wired Switch Pro mode (S3) now supports selectable USB identity profiles via `/mode` (`wired_switch_usb_identity`: `switch_pro_compat` or `switch2_pro`) with persisted preference and status visibility (`wired_switch_usb_identity_*` fields). USB identity changes require reconnect/reboot to re-enumerate.
