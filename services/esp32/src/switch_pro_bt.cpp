...
static uint8_t kReply04[] = {
    // Trigger elapsed times for L/R/ZL/ZR/SL/SR/Home. Filled dynamically.
    // Layout: report preamble + 7 little-endian uint16 trigger times.
...
static uint8_t kReply1050[] = {
    0x21, 0x04, 0x8E, 0x84, 0x00, 0x12, 0x01, 0x18, 0x80, 0x01, 0x18, 0x80,
    0x80, 0x90, 0x10, 0x50, 0x60, 0x00, 0x00, 0x10,
    0x32, 0x31, 0x32, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff,
...
static uint8_t kReply103D[] = {
    0x21, 0x05, 0x8E, 0x84, 0x00, 0x12, 0x01, 0x18, 0x80, 0x01, 0x18, 0x80,
    0x80, 0x90, 0x10, 0x3D, 0x60, 0x00, 0x00, 0x13,
...
static uint8_t kReply1080[] = {
    0x21, 0x04, 0x8E, 0x84, 0x00, 0x12, 0x01, 0x18, 0x80, 0x01, 0x18, 0x80,
    0x80, 0x90, 0x10, 0x80, 0x60, 0x00, 0x00, 0x18,
    0x5e, 0x01, 0x00, 0x00, 0xf1, 0x0f, 0x19, 0xd0, 0x4c, 0xae,
    0x40, 0xe1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff,
    0xff, 0xff, 0xff, 0xff, 0x00, 0x00
};
static uint8_t kReply1098[] = {
    0x21, 0x04, 0x8E, 0x84, 0x00, 0x12, 0x01, 0x18, 0x80, 0x01, 0x18, 0x80,
    0x80, 0x90, 0x10, 0x98, 0x60, 0x00, 0x00, 0x12,
...
bool SwitchProBT::begin() {
    ...
    // esp_hidd_dev_init(ESP_HID_TRANSPORT_BT, ...) requires Classic BT HID
    // device support in sdkconfig. On current ESP-IDF this is
    // CONFIG_BT_HID_DEVICE_ENABLED.
    Serial.println("[SwitchProBT] BT HID device support is not enabled (CONFIG_BT_HID_DEVICE_ENABLED).");
    return false;
#endif
...
void SwitchProBT::handleOutputReport(uint8_t reportId, const uint8_t* data, uint16_t len) {
    ...
    switch (subcmd) {
        ...
        case 0x04: {
            // Trigger elapsed times are used by the Switch pairing flow (L/R prompt).
            // Use a shared elapsed clock for all trigger groups, matching established
            // Switch BT emulator implementations.
            uint16_t t = static_cast<uint16_t>((millis() / 10) & 0xFFFF);
            uint8_t lo = static_cast<uint8_t>(t & 0xFF);
            uint8_t hi = static_cast<uint8_t>((t >> 8) & 0xFF);
            for (size_t i = 15; i + 1 < sizeof(kReply04); i += 2) {
                kReply04[i] = lo;
                kReply04[i + 1] = hi;
            }
            sendSubcommandReply(kReply04, sizeof(kReply04));
            break;
        }
        ...
        case 0x10: {
            ...
            switch (addr) {
                ...
                case 0x6050: sendSubcommandReply(kReply1050, sizeof(kReply1050)); break;
                case 0x6080: sendSubcommandReply(kReply1080, sizeof(kReply1080)); break;
                case 0x6098: sendSubcommandReply(kReply1098, sizeof(kReply1098)); break;
                case 0x603D: sendSubcommandReply(kReply103D, sizeof(kReply103D)); break;
